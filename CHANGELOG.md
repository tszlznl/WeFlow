...
**Jev 只有 Noul/Choice/Score，不是生成式 LLM**——所以 Agent 不是会自主规划的通用
agent，而是**有界的工具路由器**：决策接口只会「选工具」，执行在有界循环里。
**257 项测试全过**（73 + 24 + 15 + 21 + 29 + 48 + 47）。

### 新增

- **`agent` 题集 + 工具表**（`jev/agentQuestions.ts`）— 工具表五个工具：
  标注 / 待办 / 日记 / 该回吗 / 起草。决策接口的 criteria **就是工具表里的 description**，
  工具能力变化只改一处；测试断言 criteria key 和工具 id 完全对齐，不会出现「决策接口能选、
  后端没有」的幽灵工具。`hasSideEffects` 标了写收件箱的那两个（待办/日记）。
- **`findAgentTool()`** — 命令式入口：`/todo` `/待办` `/该回吗？` 这类显式命令直接定位工具，
  **跳过决策调用**，不花钱、不需要 key 也能预测行为。中英文别名、大小写、斜杠/问号/感叹号、
  首尾空格都归一化；乱写给 null。
- **`jevService.runAgent()`** — 两段式：
  - **排计划**：每轮只见一步，把已排工具喂回 state 问下一轮，`none` 或重复工具就停；
    硬上限 `MAX_AGENT_STEPS=5`，决策接口抽风也不会无限编排。
  - **确认门**：**只有决策接口选的工具才问**。显式命令是用户自己点的名，再问一遍纯属啰嗦。
  - **执行**：按计划顺序跑，一步硬失败就停（后面的工具多半依赖前面的结果）。
  - 副作用工具没确认时只返回 `{needsConfirm, plan, confirmPrompt}`，前端问完带
    `confirmed:true` 重跑——决策命中缓存，不重复执行工具。
  - 消息键复用前端 `getMessageKey` 的公式（`messageKeyOf` 私有方法），待办去重靠它，
    两边算不一致就会重复入库。
- **IPC `jev:runAgent`** + preload + 类型（`AgentRunResult` / `AgentStepResult`）。
- 前端：会话详情面板「Jev 命令」区——命令输入框 + 执行按钮 + 确认条（橙边，确认执行/取消）
  + 逐步执行结果（成功绿条/失败红条）+ 截断提示。
- `npm run test:jev:phase5` — 47 项：题集/工具表对齐 / 命令归一化 / 直接命令跳过决策 /
  确认门（不确认只出计划、确认后入库）/ 只读工具不确认 / 重复工具自然停 / 无 key 拒绝 /
  死端点不执行 / 幽灵工具不认 / 全 none 给提示 / 空命令拒绝。本地 mock 服务器，全程不联网。

### 决策

- **不做自主规划 agent**：Jev 没有生成题，让它写「我先去查一下然后帮你写邮件」这种自由文本
  计划是强人所难。Agent 的智能体现在「选工具」这一步，执行是确定性的。
- **确认门不做成判断题**：该不该问是工具属性（有没有副作用）决定的，花一次调用去问
  「该不该问」纯属浪费。
